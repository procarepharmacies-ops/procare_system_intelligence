...
import { getBranchTransfers } from './api';
...
const BranchTransfer = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      try {
        const result = await getBranchTransfers(source);
    ...
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      <div className="flex justify-between items-center bg-card p-6 rounded-2xl border border-border shadow-sm">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-foreground">تحويل نقدي لفرع</h1>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm text-right">
            <thead className="bg-muted/50 text-muted-foreground">
              <tr>
                <th className="px-6 py-4 font-medium">رقم التحويل</th>\n                <th className="px-6 py-4 font-medium">التاريخ</th>\n                <th className="px-6 py-4 font-medium">المبلغ</th>\n                <th className="px-6 py-4 font-medium">من فرع</th>\n                <th className="px-6 py-4 font-medium">إلى فرع</th>\n                <th className="px-6 py-4 font-medium">ملاحظات</th>
              </tr>
            </thead>
              ...
              {loading ? (
                ...
                data.map((item, idx) => (
                  <tr key={idx} className="hover:bg-muted/50 transition-colors">
                    <td className="px-6 py-4">{item.convert_id || "-"}</td>\n                    <td className="px-6 py-4">{item.convert_date || "-"}</td>\n                    <td className="px-6 py-4">{item.amount || "-"}</td>\n                    <td className="px-6 py-4">{item.from_branch_id || "-"}</td>\n                    <td className="px-6 py-4">{item.to_branch_id || "-"}</td>\n                    <td className="px-6 py-4">{item.notes || "-"}</td>
                  </tr>
                ))
              )}
            </tbody>
