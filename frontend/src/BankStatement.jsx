...
import { getBankStatement } from './api';
...
const BankStatement = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      try {
        const result = await getBankStatement(source);
    ...
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      <div className="flex justify-between items-center bg-card p-6 rounded-2xl border border-border shadow-sm">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-foreground">كشف حساب بنكي</h1>
              ...
              {loading ? (
                ...
                data.map((item, idx) => (
                  <tr key={idx} className="hover:bg-muted/50 transition-colors">
                    <td className="px-6 py-4">{item.trans_id || "-"}</td>\n                    <td className="px-6 py-4">{item.trans_date || "-"}</td>\n                    <td className="px-6 py-4">{item.bank_id || "-"}</td>\n                    <td className="px-6 py-4">{item.money_amount || "-"}</td>\n                    <td className="px-6 py-4">{item.notes || "-"}</td>
                  </tr>
                ))
              )}
            </tbody>
