...
import { getVendorBalances } from './api';
...
const VendorBalances = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      try {
        const result = await getVendorBalances(source);
    ...
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      <div className="flex justify-between items-center bg-card p-6 rounded-2xl border border-border shadow-sm">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-foreground">الأرصدة الافتتاحية للموردين</h1>
              ...
              {loading ? (
                ...
                data.map((item, idx) => (
                  <tr key={idx} className="hover:bg-muted/50 transition-colors">
                    <td className="px-6 py-4">{item.vendor_id || "-"}</td>\n                    <td className="px-6 py-4">{item.vendor_name_ar || "-"}</td>\n                    <td className="px-6 py-4">{item.vendor_balance || "-"}</td>\n                    <td className="px-6 py-4">{item.active || "-"}</td>
                  </tr>
                ))
              )}
            </tbody>
