...
import { getSalesEmployee } from './api';
...
const SalesEmployee = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      try {
        const result = await getSalesEmployee(source);
    ...
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      <div className="flex justify-between items-center bg-card p-6 rounded-2xl border border-border shadow-sm">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-foreground">تقرير مبيعات الموظفين</h1>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm text-right">
            <thead className="bg-muted/50 text-muted-foreground">
              <tr>
                <th className="px-6 py-4 font-medium">رقم الكاشير</th>\n                <th className="px-6 py-4 font-medium">عدد الفواتير</th>\n                <th className="px-6 py-4 font-medium">إجمالي المبيعات</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-border">
              {loading ? (
                ...
                <tr><td colSpan="3" className="text-center py-8 text-muted-foreground">لا توجد بيانات متاحة</td></tr>
              ) : (
                data.map((item, idx) => (
                  <tr key={idx} className="hover:bg-muted/50 transition-colors">
                    <td className="px-6 py-4">{item.cashier_id || "-"}</td>\n                    <td className="px-6 py-4">{item.total_bills || "-"}</td>\n                    <td className="px-6 py-4">{item.total_sales || "-"}</td>
                  </tr>
                ))
              )}
            </tbody>
